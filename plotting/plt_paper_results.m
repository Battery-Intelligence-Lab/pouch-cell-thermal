...
if ~isempty(ch_files_found)
    ...
    for i = 1:length(ch_files_found)
        ...
        % Error calculations:
        ch_data.(data_key).RMSE.Tcold = rmse(data.Tcold(1:Exp_len), data.ExpT_cold(1:Exp_len));
        ch_data.(data_key).RMSE.Tmean = rmse(data.Tmean(1:Exp_len), data.ExpT_mean(1:Exp_len));
        ch_data.(data_key).RMSE.Thot  = rmse(data.Thot(1:Exp_len), data.ExpT_hot(1:Exp_len));
...
if ~isempty(dch_files_found)
    ...
    for i = 1:length(dch_files_found)
        ...
        dch_data.(data_key).RMSE.Tcold = rmse(data.Tcold(1:Exp_len), data.ExpT_cold(1:Exp_len));
        dch_data.(data_key).RMSE.Tmean = rmse(data.Tmean(1:Exp_len), data.ExpT_mean(1:Exp_len));
        dch_data.(data_key).RMSE.Thot  = rmse(data.Thot(1:Exp_len), data.ExpT_hot(1:Exp_len));
...
for socc = [30, 50, 70]
    rc = pulse_data.("soc" + socc + "_cr8").RMSE;
    
    fprintf('For 8C SOC = %d %% RMSE: V = %4.1f mV, Thot = %4.3f °C, Tavg = %4.3f °C, Tcold = %4.3f °C\n',...
    socc, rc.V*1e3, rc.Thot, rc.Tmean, rc.Tcold);

    avgpulse.V = avgpulse.V + rc.V/3;
    avgpulse.Thot = avgpulse.Thot + rc.Thot/3;
    avgpulse.Tmean = avgpulse.Tmean + rc.Tmean/3;
    avgpulse.Tcold = avgpulse.Tcold + rc.Tcold/3; 
end

    fprintf('For 8C avg RMSE: V = %4.1f mV, Thot = %4.3f °C, Tavg = %4.3f °C, Tcold = %4.3f °C\n',...
    avgpulse.V*1e3, avgpulse.Thot, avgpulse.Tmean, avgpulse.Tcold);


%% CC plots together:
clc; close all;
i_sel = 4; % Selected parameter set which is 70% params.
